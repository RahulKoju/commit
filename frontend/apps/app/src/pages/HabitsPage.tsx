    ...
    <div className="relative">
      ...
      {open ? (
        <form className="absolute right-0 z-20 mt-2 flex w-72 gap-2 rounded-xl border bg-background p-3 shadow-xl" onSubmit={onSubmit}>
          <input name="name" required placeholder="Mindfulness" className="h-9 min-w-0 flex-1 rounded-md border bg-background px-3 text-sm" />
    ...
    <div className="relative">
      ...
      {open ? (
        <form className="absolute right-0 z-20 mt-2 grid w-[min(92vw,28rem)] gap-3 rounded-xl border bg-background p-4 shadow-xl" onSubmit={onSubmit}>
          <input name="name" required placeholder="Meditate" className="h-9 rounded-md border bg-background px-3 text-sm" />
          <select name="category_id" required className="h-9 rounded-md border bg-background px-3 text-sm">
          ...
          <div className="grid gap-2 sm:grid-cols-3">
            <select name="type" defaultValue="boolean" className="h-9 rounded-md border bg-background px-3 text-sm">
              <option value="boolean">Boolean</option>
              <option value="numeric">Numeric</option>
            </select>
            <input name="target_value" type="number" min={0} step="0.1" placeholder="Target" className="h-9 rounded-md border bg-background px-3 text-sm" />
            <input name="target_unit" placeholder="Unit" className="h-9 rounded-md border bg-background px-3 text-sm" />
          </div>
          <div className="grid gap-2 sm:grid-cols-2">
            <select name="frequency_type" defaultValue="daily" className="h-9 rounded-md border bg-background px-3 text-sm">
              <option value="daily">Daily</option>
              <option value="weekly">Specific days</option>
            </select>
            <input name="weekly_goal" type="number" min={1} defaultValue={7} className="h-9 rounded-md border bg-background px-3 text-sm" />
          </div>
          <textarea name="description" rows={2} placeholder="Description" className="rounded-md border bg-background px-3 py-2 text-sm" />
  ...
  return (
    <article className={`rounded-xl border p-4 ${selected ? "bg-muted" : "bg-background"}`} onClick={onSelect}>
      ...
      {habit.type === "boolean" ? (
        ...
        <form className="mt-4 grid gap-2" onSubmit={onNumericSubmit}>
