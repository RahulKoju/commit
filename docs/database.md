...
All tables use UUID primary keys (generated via `gen_random_uuid()` from the `pgcrypto` extension) and `TIMESTAMPTZ` timestamps.
...
Indexes: `idx_habit_categories_user_id(user_id)`

---

### `topics`

User-defined subjects for learning, task association, and note categorization.
...
Constraints: `UNIQUE(user_id, name)`
Indexes: `idx_topics_user_id(user_id)`
...
Todo-style tasks with priority, scheduling, and status tracking.
...
Trackable habits of type `boolean` (done/not done) or `numeric` (quantifiable).
...
- `topics` is a central entity referenced by `tasks`, `learn_entries`, `focus_sessions`, and (via `note_topics`) `notes`.
- `habit_categories` groups `habits`; deleting a category is restricted (`ON DELETE RESTRICT`) if habits reference it.
- `habits` has daily logs in `habit_logs` (one row per habit per date).
- `notes` has a many-to-many relationship with `topics` via the `note_topics` junction table.
